...
class AlexNet(nn.Module):
    ...
    def __init__(self, num_classes=NUM_CLASSES):
        ...
        self.branch1 = nn.Sequential(OrderedDict([
            ...
            ("norm0", nn.LocalResponseNorm(size=3, alpha=0.00005, beta=0.75)),
            ('convB1', nn.Conv2d(64, 32, 3, padding=1, stride=1)),
            ('relu1', nn.ReLU()),
            ("convB2", nn.Conv2d(32, 32, 3, padding=1, stride=1)),
            ("relu2", nn.ReLU()),
...
if __name__ == '__main__':
    ...
    for epoch in range(NUM_EPOCHS):
        for imgs, classes in dataloader:
            ...
            total_steps += 1
        if end:
            torch.save(alexnet.state_dict(), CHECKPOINT_DIR+'best_model.pt')
            break
        lr_scheduler.step()
